...
import { BuyerDeleteDiscussAPI, BuyerDeletePostAPI, BuyerGetDiscussAPI, BuyerGetPostListAPI, BuyerSendNewPostAPI } from '@/apis/Buyer';
...
const getDiscuss = async () => {
  const temp = await BuyerGetDiscussAPI(route.params.id);
...
{
  await BuyerDeleteDiscussAPI(route.params.id)
  // console.log("删除成功")
  ElMessage({
    message: '删除成功！',
    type: 'success',
...
const BuyerSendNewPost = async () => {
  ...
  // const temp = await BuyerSendNewPostAPI({ account, date, postContent.value })
}

const postList = ref([])
const buyerGetPostList = async() => {
  const id = route.params.id
  const temp = await BuyerGetPostListAPI(id)
  ...
  if (buyerInfo.account != postById)
{
  ...
  // await BuyerDeletePostAPI(postId, postById);
  console.log(postId, postById)
  ElMessage({
    message: '删除成功！',
    type: 'success',
  })
  // router.replace({ path: `/BuyerDiscuss/${route.params.id}` })
    setTimeout(() => {
      ...
      <el-container class="main">
        ...
        <el-main style="padding-left: 10%; padding-right: 10%">
          ...
          <el-dialog v-model="dialogFormVisible">
            <el-input class="input" type="textarea" :rows="4" placeholder="与主题相关的讨论" v-model="postContent">
            </el-input>
            <div slot="footer" class="dialog-footer">
              <el-button @click="dialogFormVisible = false">取 消</el-button>
              <el-button type="primary" @click="BuyerSendNewPost">确 定</el-button>
            </div>
          </el-dialog>
